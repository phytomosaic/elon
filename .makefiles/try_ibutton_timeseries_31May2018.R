######################################################################
# trials for ELON iButtons
#  Rob Smith, smithr2@oregonstate.edu, Oregon State Univ, 15 May 2018
...
###   load data, acquired 15-16 May 2018
cols <- inferno(7, alpha=0.7)
pth <- 'C:/Users/Rob/Documents/_prj/7_elon/data/trials_ibutton_16May2018'
f <- '/trials_ibutton_21May2018.csv'
d <- read.csv(file=paste0(pth, f), header=T, stringsAsFactors=F)

# f1 <- '/trials_ibutton_16May2018.csv'
# f2 <- '/trials_ibutton_21May2018.csv'
# f3 <- '/trials_ibutton_31May2018.csv'
# d <- rbind(read.csv(file=paste0(pth, f1), header=T, stringsAsFactors=F),
#            read.csv(file=paste0(pth, f2), header=T, stringsAsFactors=F),
#            read.csv(file=paste0(pth, f3), header=T, stringsAsFactors=F))
# rm(f1,f2,f3)
...
z$t_diff  <- z[,1] - z[,3]
z$rh_diff <- z[,2] - z[,4]
plot(z, screens=c(1,2,1,2,3,4), col=c(1,1,2,2,1,1),
     las=1, mgp=c(2.5,.7,0), oma=c(2,0,0,0), main='',
     ylab=c('T (C°)','RH (%)','T diff','RH diff'))

# calc daily temperature range (not really continentality)
bys <- as.POSIXct(as.character(index(z)),
                  format='%Y-%m-%d %H') # by hourly index
...
dtr <- zoo(sapply(z,  FUN=f2), unique(bys))
dtr <- zoo(sapply(dtr, FUN=f3), time(dtr))[,c(1,3)]
dtr$t_diff  <- dtr[,1] - dtr[,2]
# plot(dtr, las=1)


# tiff('./fig_01_ibuttontrials_30May2018.tif',wid=7.5,hei=3.5,
#      unit='in',res=400, compr='lzw')
plot(z[,c(1,3,5)], screens=c(1,1,2), col=c(1,2,1),
     las=1, mgp=c(2.5,.7,0), oma=c(2,0,0,0), main='',
     ylab=c('Temperature (C°)','Deviation (C°)'))
# dev.off()
# red line = unshielded, black line = radshield
# conclude: unshielded higher T and lower RH than shielded.
# deviation most pronounced in direct sun before it swung behind bldg
# conclude: must use radshield.

# tiff('./fig_02_ibuttontrials_30May2018.tif',wid=7.5,hei=3.5,
#      unit='in',res=400, compr='lzw')
plot(dtr[,1:2], screens=c(1,1), col=c(1,2),
     las=1, mgp=c(2.5,.7,0), oma=c(2,0,0,0),
     main='', xlab='', ylab='Hourly temp range (C°)')
# dev.off()



# TODO: calculate diurnal temperature range, diurnal variance, max/min





### wavelets coherence analysis
require(biwavelet)
head(z)
plot(z)

# convert back to matrix
t1 <- cbind(index(z), as.numeric(z[,1]))  # Temperature, w/shield
t2 <- cbind(index(z), as.numeric(z[,3]))  # Temperature, w/o shield
...
# plot power
par(oma=c(0,0,0,1), mar=c(5,4,4,5)+.1, las=1)
plot(wt.t1, plot.cb=TRUE, plot.phase=FALSE)

# cross-wavelet
xwt.t1t2 <- xwt(t1, t2)
plot(xwt.t1t2, plot.cb=T, plot.phase=T, lwd.sig=.5, arrow.len=.07)

# wavelet coherence - nrands should be large (>= 999)
wtc.t1t2 <- wtc(t1, t2, nrands=9, sig.test=0)

# plot two time-series and their coherence
layout(matrix(1:2,2,1),hei=c(1,2))
par(oma=c(0,0,0,0), mar=c(2,4.1,.5,.1), las=1, bty='l')
plot(z[,1], ylim=c(8,30), ylab='Temperature (C°)', lwd=.5)
lines(z[,3], col=2, lwd=.5)
plot(wtc.t1t2, plot.cb=F, plot.phase=T, lwd.sig=.5, arrow.len=.07)

###  end wavelets coherence analysis
...
bymon <- as.yearmon(index(z))
monavg <- zoo(sapply(z,  FUN=f4), unique(bymon)) # monthly avg T
td <- as.numeric(monavg[11,]) - as.numeric(monavg[4,])
# adjust continentality by mean (sort of like a CV)
mat <- sapply(z, FUN=mean, na.rm=T)
tdmat <- td/mat
...
axis(1, at=1:6, labels=colnames(z))
...
# z <- zoo(dp[,keep], dtp)
# plot(z, screens=1, col=inferno(6, alpha=0.4), ylim=c(-10, 35),
#      las=1, bty='l')
# colMeans(z, na.rm=T)
